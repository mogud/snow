...
type iService interface {
	...
	Entry(ctx node.IRpcContext, funcName string, argGetter func(ft reflect.Type) ([]reflect.Value, error)) func()
...
func (ss *Service) delayEntry(ctx node.IRpcContext, funcName string, argGetter func(ft reflect.Type) ([]reflect.Value, error)) {
	if f := ss.realSrv.Entry(ctx, funcName, argGetter); f != nil {
		ss.delayedRPCs = append(ss.delayedRPCs, f)
	}
}

func (ss *Service) entry(ctx node.IRpcContext, funcName string, argGetter func(ft reflect.Type) ([]reflect.Value, error)) {
	if f := ss.realSrv.Entry(ctx, funcName, argGetter); f != nil {
		f()
	}
}

func (ss *Service) Entry(ctx node.IRpcContext, funcName string, argGetter func(ft reflect.Type) ([]reflect.Value, error)) func() {
...
func (ss *Service) Tick(interval, delay time.Duration, f func()) node.ITimer {
	return ss.timeWheel.Tick(interval, delay, f)
}

func (ss *Service) TickDelayRandom(interval time.Duration, f func()) node.ITimer {
	delay := time.Duration(rand.Int63n(int64(interval)))
	return ss.timeWheel.Tick(interval, delay, f)
}

func (ss *Service) TickAfter(interval time.Duration, f func(func())) node.ITimer {
...
func (ss *Service) After(delay time.Duration, f func()) node.ITimer {
...
func (ss *Service) CreateProxyDefault() node.IProxy {
	return newEmptyServiceProxy()
}

func (ss *Service) CreateProxy(name string) node.IProxy {
	kind := gNode.name2Info[name].Kind
	return newServiceProxy(ss, nil, -1, -kind)
}

func (ss *Service) CreateProxyByNodeKind(naddr node.INodeAddr, name string) node.IProxy {
	kind := gNode.name2Info[name].Kind
	return newServiceProxy(ss, nil, naddr.(NodeAddr), -kind)
}

func (ss *Service) CreateProxyByNodeAddr(naddr node.INodeAddr, saddr int32) node.IProxy {
	return newServiceProxy(ss, nil, naddr.(NodeAddr), saddr)
}

func (ss *Service) CreateProxyByUpdaterKind(naddrUpdater *NodeAddrUpdater, name string) node.IProxy {
	kind := gNode.name2Info[name].Kind
	return newServiceProxy(ss, naddrUpdater, 0, -kind)
}

func (ss *Service) CreateProxyByUpdaterAddr(naddrUpdater *NodeAddrUpdater, saddr int32) node.IProxy {
	return newServiceProxy(ss, naddrUpdater, 0, saddr)
}

func (ss *Service) RpcStatus(ctx node.IRpcContext) {
	ctx.Return("OK")
}

func (ss *Service) RpcReload(ctx node.IRpcContext) {
	ctx.Return()
}
