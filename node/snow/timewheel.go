...
func (ss *timeWheel) CreateTimer() node.ITimer {
	return &timer{
		tw: ss,
		ti: &timerItem{
			idx: ss.genIdx(),
...
func (ss *timeWheel) Tick(interval, delay time.Duration, f func()) node.ITimer {
	t := ss.CreateTimer()
...
func (ss *timeWheel) After(delay time.Duration, f func()) node.ITimer {
	t := ss.CreateTimer()
	t.Set(0, delay, f)
	t.Start()
	return t
