...
func (ss *dumbPromise) Then(f interface{}) node.IPromise {
	return ss
}

func (ss *dumbPromise) Catch(f func(error)) node.IPromise {
	return ss
}

func (ss *dumbPromise) Final(f func()) node.IPromise {
	return ss
}

func (ss *dumbPromise) Timeout(timeout time.Duration) node.IPromise {
	return ss
}
...
func (ss *promise) Then(f interface{}) node.IPromise {
	ss.succCb = append(ss.succCb, f)
	return ss
}

func (ss *promise) Catch(f func(error)) node.IPromise {
	ss.errCb = f
	return ss
}

func (ss *promise) Final(f func()) node.IPromise {
	ss.finalCb = f
	return ss
}

func (ss *promise) Timeout(timeout time.Duration) node.IPromise {
	ss.timeout = timeout
	return ss
}
...
var _ = node.IProxy((*serviceProxy)(nil))
...
func (ss *serviceProxy) Call(fname string, args ...interface{}) node.IPromise {
	if ss.saddr == 0 {
...
func (ss *serviceProxy) GetNodeAddr() node.INodeAddr {
	if ss.saddr == 0 {
...
func (ss *serviceProxy) Reset(proxy node.IProxy) {
	if proxy == nil {
		ss.saddr = 0
		return
	}
	...
	if len(p.succCb) == 0 {
		...
		m.cb = func(mm *message) {
			srv.Fork("proxy.forkcb", func() {
				...
				fret := fv.Call(ret)
				for len(p.succCb) > 0 {
					if ft.NumOut() == 1 && ft.Out(0) == reflect.TypeOf((*node.IPromise)(nil)).Elem() {
						if fret[0].IsNil() {
							break
						}
